...
class AlphaIntelligence(av):
    ...
    @av._output_format
    @av._call_api_on_func
    def get_earnings_call_transcript(self, symbol=None, quarter=None):
        """ returns the earnings call transcript for a given company in a specific quarter,
        covering over 15 years of history and enriched with LLM-based sentiment signals.

        Keyword Arguments:
            symbols:  the stock/crypto/forex symbols of your choice
            quarter: Fiscal quarter in YYYYQM format. Any quarter since 2010Q1 is supported.
        """
    ...
    @av._output_format
    @av._call_api_on_func
    def get_insider_transactions(self, symbol=None):
        """ Returns the latest and historical insider transactions
        made by key stakeholders (e.g., founders, executives, board members, etc.)
        of a specific company.

        Keyword Arguments:
            symbol:  the stock/crypto/forex symbols of your choice
        """
        _FUNCTION_KEY = "INSIDER_TRANSACTIONS"
